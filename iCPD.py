...
def updateRegularizationParameter(ztz, sketchingRate):
    '''
    # SV = singular value
    maxSV = np.linalg.norm(ztz, ord=2)
    minSV = np.linalg.norm(ztz, ord=-2)
    print 'Max SV =',maxSV
    print 'Min SV =',minSV
    newlambda = maxSV / minSV
    print 'Condition # (Max/Min) =',newlambda
#    return newlambda
    return minSV
    '''
    '''
    # values for R=5 ill-conditioned tensor
    if sketchingRate == 0.0001:
	return 0.1
    elif sketchingRate == 0.000001:
	return 0.001
    elif sketchingRate == 1:
	return 1
    else:
	return 0.01
    '''
    # values for R=50 ill-conditioned tensor
    if sketchingRate == 0.0001:
	return 0.0001
    elif sketchingRate == 0.000001:
	return 0.0000001
...
#	if sketching==1 or sketching>=3:
	...
	'''
	# paige
	regulNorm = norm(np.dot(A,ZTZ) - XZ, 2)
	print step,'Regul norm A =',regulNorm
    if sketchingRate == 0.0001:
	return 0.1
    elif sketchingRate == 0.000001:
	return 0.001
...
#       if (step%5 == 0) and sketching==12:
#        if (step == 3) and sketching==12:
	'''
        if ((step == 3) or (step > 0 and step%10==0)) and sketching==12:
            gradientNorm = norm(np.dot(A,ZTZ) - XZ, 2)
            minErr = 100
            decompMode = 3
	    A_start = A
            for exp in range(0,10):
                print 'trying exp=',exp
                print 'which is =',2*(10**(-1*exp))
                regulParamWeight = 2*(10**(-1*exp))
                regulParam = regulParamWeight * gradientNorm
                if regularization > 0:
                    ZTZ = ZTZ + regulParam * eye
                if regularization == 2:
                    XZ = XZ + regulParam * A_start
                A = solve(ZTZ.T, XZ.T).T
                errorRDD = tensorRDD.mapPartitions(singleModeALSstep)
                sums = errorRDD.reduceByKeyLocally(add)
                errT = np.sqrt(sums['error'] / normX)
                print 'errT =',errT
                if errT < minErr:
                    minErr = errT
                    regulParamWeight_hold = 2*(10**(-1*exp))
                    A_hold = A
            print 'REGUL PARAM WEIGHT =',regulParamWeight_hold
            regulParamWeight = regulParamWeight_hold
            A = A_hold
	'''
...
#	if sketching==1 or sketching>=3:
	...
	# paige
	# need to calculate error for tensor 
#	if sketching == 13 and 1 <= step and (step % 2) == 0:
#	if sketching == 13 and 1 <= step and (step % 4) == 0:
#	if sketching == 13 and 1 <= step <= 6:
#	if sketching in (13,14):
	'''
	if sketching in (13,14) and (1 <= step <= 6 or (step > 0 and (step % 5) == 0)):
	    # deactivate regularization
	    hold_regulParam = regulParam
	    regulParam = 0

	    # Evaluate error CPD-sketched for (A,B) and (start_A,start_B) = delta m
	    decompMode = 3
	    errorRDD = tensorRDD.mapPartitions(singleModeALSstep)
	    sums = errorRDD.reduceByKeyLocally(add)
	    m_New = np.sqrt(sums['error'] / normX)
	    ...
	    errorRDD = tensorRDD.mapPartitions(singleModeALSstep)
	    sums = errorRDD.reduceByKeyLocally(add)
	    m_Old = np.sqrt(sums['error'] / normX)

	    # deactivate sketching
	    hold_errorCalcSketchingRate = errorCalcSketchingRate
	    errorCalcSketchingRate = 0
	    hold_sketching = sketching
	    sketching = 0
	    hold_sketchingRate = sketchingRate
	    sketchingRate = 0

	    # Evaluate error CPD-ALS for (A,B) and (start_A,start_B) = delta f
	    errorRDD = tensorRDD.mapPartitions(singleModeALSstep)
	    sums = errorRDD.reduceByKeyLocally(add)
	    f_Old = np.sqrt(sums['error'] / normX)
	    ...
	    errorRDD = tensorRDD.mapPartitions(singleModeALSstep)
	    sums = errorRDD.reduceByKeyLocally(add)
	    f_New = np.sqrt(sums['error'] / normX)

	    # take ratio (rho_k) and use to determine next regularization parameter
	    # rho = actual reduction / predicted reduction for traditional ALS
	    rho_k = (f_Old - f_New) / (m_Old - m_New)

	    if rho_k <= 0.25:
		regulParamWeight = min(10.0 * regulParamWeight, maxRegulParamWeight)
	    elif rho_k >= 0.75:
		regulParamWeight = 0.1 * regulParamWeight
	    print 'rho =',rho_k,'; regulParamWeight =',regulParamWeight

	    sketching = hold_sketching
	    sketchingRate = hold_sketchingRate
	    regulParam = hold_regulParam
	    errorCalcSketchingRate = hold_errorCalcSketchingRate
	'''
